...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header with-border">
                ...
                <div class="card-tools">
                    <div class="btn-group float-right mr-5">
                        <a target=_new href="{{ sc_route_admin('admin_mvendor_product.index') }}" class="btn  btn-flat btn-default" title="List">
            ...
            <!-- form start -->
            <form action="{{ sc_route_admin('admin_mvendor_product.create') }}" method="post" name="form_name" accept-charset="UTF-8" 
                ...
                <div id="main-add" class="card-body">
                        ...
                        <div class="form-group row kind kind0 kind1  {{ $errors->has('vendor_category_id') ? ' text-red' : '' }}">
                            <label for="vendor_category_id"
                                class="col-sm-2 col-form-label">{{ sc_language_render('product.category_store') }}</label>
                            <div class="col-sm-8">
                                <select class="form-control input-sm vendor_category_id select2" 
                                data-placeholder="{{ sc_language_render('product.admin.select_category') }}" style="width: 100%;"
                                    name="vendor_category_id">
                                    <option value=""></option>
                                    @foreach ($categoriesStore as $k => $v)
                                    <option value="{{ $k }}" {{ (old('vendor_category_id') == $k) ? 'selected':'' }}>{{ $v }}
                                    </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('vendor_category_id'))
                                <span class="form-text">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('vendor_category_id') }}
                                </span>
                                @endif
                            </div>
                        </div>
...
@if (sc_config_admin('product_attribute'))
                        ...
                        <div class="form-group kind  row">
                            <div class="col-sm-2">
                                <label>{{ sc_language_render('product.attribute') }}</label>
                            </div>
                            <div class="col-sm-8">
...
@push('scripts')
@include($templatePathAdminVendor.'component.ckeditor_js')
...
$('textarea.editor').ckeditor(
    {
        filebrowserImageBrowseUrl: '{{ sc_route_admin('vendor_admin.home').'/'.config('lfm.url_prefix') }}?type=product',
        filebrowserImageUploadUrl: '{{ sc_route_admin('vendor_admin.home').'/'.config('lfm.url_prefix') }}/upload?type=product&_token={{csrf_token()}}',
        filebrowserBrowseUrl: '{{ sc_route_admin('vendor_admin.home').'/'.config('lfm.url_prefix') }}?type=Files',
        filebrowserUploadUrl: '{{ sc_route_admin('vendor_admin.home').'/'.config('lfm.url_prefix') }}/upload?type=file&_token={{csrf_token()}}',
