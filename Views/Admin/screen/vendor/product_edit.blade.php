...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form action="{{ sc_route_admin('admin_mvendor_product.edit',['id'=>$product['id']]) }}" method="post" accept-charset="UTF-8"
...
@if (sc_config_global('MultiVendor'))
                        {{-- Sub category --}}
                        <div class="form-group row {{ $errors->has('vendor_category_id') ? ' text-red' : '' }}">
                            <label for="vendor_category_id" class="col-sm-2 col-form-label">{{ sc_language_render('product.category_store') }}</label>
                            <div class="col-sm-8">
                                <select class="form-control vendor_category_id select2" style="width: 100%;"
                                    name="vendor_category_id">
                                    <option value=""></option>
                                    @foreach ($categoriesStore as $k => $v)
                                    <option value="{{ $k }}"
                                        {{ (old('vendor_category_id', $vendor_category_id) ==$k ) ? 'selected':'' }}>
                                        {{ $v }}</option>
                                    @endforeach
                                </select>
...
@if (sc_config_admin('product_attribute'))
                    @if ($product->kind == SC_PRODUCT_SINGLE)
                        ...
                        <div class="form-group row">
                            <div class="col-sm-2">
                                <label>{{ sc_language_render('product.attribute') }}
                            </div>
                            <div class="col-sm-8">
